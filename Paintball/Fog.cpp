...
AFog::AFog() :m_wholeTextureRegion(0, 0, 0, 0, m_textureSize, m_textureSize) {
	...
	// Load the base material from your created material.
	{
		static ConstructorHelpers::FObjectFinder <UMaterial>
			asset(TEXT("Material'/Game/Shader/FogMat.FogMat'"));
...
void AFog::UpdateTextureRegions(UTexture2D * Texture, int32 MipIndex, uint32 NumRegions,
	...
	FUpdateTextureRegionsData *, RegionData, RegionData, bool, bFreeData, bFreeData, {
		for (uint32 RegionIndex = 0; RegionIndex < RegionData->NumRegions; ++RegionIndex) {
			...
			if (RegionData->MipIndex >= CurrentFirstMip) {
				RHIUpdateTexture2D(RegionData->Texture2DResource->GetTexture2DRHI(), RegionData 
					-> MipIndex - CurrentFirstMip, RegionData->Regions[RegionIndex], RegionData->SrcPitch, RegionData 
					-> SrcData + RegionData->Regions[RegionIndex].SrcY * RegionData->SrcPitch + RegionData 
